...
class Poly {
	...
	public static function arrayFromBytes(bytes:Bytes, data_version:WadfileDataVersion):Array<Poly> {
		var out = [];
		...
		switch (data_version) {
			case Marathon:
				for (p in out) {
					p.media_index = 0;
					p.floor_origin_x = p.floor_origin_y = 0;
					p.ceiling_origin_x = p.ceiling_origin_y = 0;
				}
			default:
		}
		return out;
